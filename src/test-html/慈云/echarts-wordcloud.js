				...
				ecModel.eachSeriesByType('wordCloud', function(seriesModel) {
					...
					wordCloudLayoutHelper(canvas, {
						list: data.mapArray('value', function(value, idx) {
							...
							return [
								data.getName(idx),
								itemModel.get('textStyle.normal.textSize',
									true) ||
								echarts3.number.linearMap(value,
									valueExtent, sizeRange),
								idx
							];
						}).sort(function(a, b) {
			...
			layout.mergeLayoutParam = function(targetOption, newOption, opt) {
				...
				function merge(names) {
					...
					each(names, function(name) {
						...
						hasProp(newOption, name) && (newParams[name] = merged[name] = newOption[
							name]);
									...
									if (idx !== 2) {
										...
										return (
											...
											Math.round(
												...
												size : (otherModel.getShallow(name,
													true) || size)
											)
										) + 'px';
									}
			...
			(function(global) {
				...
				var WordCloud = function WordCloud(elements, options) {
					...
					var getRotateDeg = function getRotateDeg() {
						...
						return minRotation + Math.round(Math.random() * rotationRange /
										...
										while (x--) {
											if (imageData[((gy * g + y) * width +
												...
												if (debug) {
													...
													fctx.fillRect(gx * g, gy * g, g - 0.5, g -
														0.5);
												}
						...
						elements.forEach(function(el) {
							if (el.getContext) {
								...
								ctx.font = settings.fontWeight + ' ' +
									(fontSize * mu).toString(10) + 'px ' + settings
									.fontFamily;
								ctx.fillStyle = color;
								...
								var transformRule = '';
								transformRule = 'rotate(' + (-rotateDeg / Math.PI * 180) +
									'deg) ';
								if (info.mu !== 1) {
									transformRule +=
								...
								var styleRules = {
									...
									'font': settings.fontWeight + ' ' +
										(fontSize * info.mu) + 'px ' + settings
										.fontFamily,
									'left': ((gx + info.gw / 2) * g + info
										.fillTextOffsetX) + 'px',
									'top': ((gy + info.gh / 2) * g + info
										.fillTextOffsetY) + 'px',
					...
					var start = function start() {
						...
						center = (settings.origin) ? [settings.origin[0] / g, settings.origin[1] /
							g] : [ngx / 2, ngy / 2];
						...
						if (settings.hover || settings.click) {
							...
							canvas.addEventListener('wordcloudstart', function stopInteraction() {
								canvas.removeEventListener('wordcloudstart',
									stopInteraction);
						...
						var timer = loopingFunction(function loop() {
							if (i >= settings.list.length) {
								...
								removeEventListener('wordcloudstart',
				...
				if (true) {
					!(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_RESULT__ = function() {
							return WordCloud;
						}.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__), __WEBPACK_AMD_DEFINE_RESULT__ !==
