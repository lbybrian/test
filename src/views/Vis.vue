<!--<template>
	<div id="visTimeline" class="visTimeline"></div>
</template>
...
<template>
	<div>
		<el-row>
			<div class="vis-box time_line_box">
		<div style="display: inline-flex;margin: 10px 0;">
			...
			<img class="vis-screen-img" :src="visScreenImg" @click="unfold()" style="width: 40px;height: 40px;margin-left: 20px;" />
		</div>
		...
		<el-row>
			<VisTimeLine ref="timeline" @click="clickTimeline" style= "width:100%;height:200px;margin-top: 30px;"></VisTimeLine>
		</el-row>
	</div>
...
<script>
	...
	import VisTimeLine from '@/components/vis-timeline.vue'
	import VisTree from '@/components/vis-tree.vue'
	import VisTree2 from '@/components/vis-tree2.vue'
	export default {
		name: "vis",
		components: {
			VisTimeLine,
		...
		computed: {
			visScreenImg: function() {
				if(this.clickSign) {
					return require('/static/imgs/收起.png');
				} else {
					return require('/static/imgs/展开.png');
				}
			}
		},
		methods: {
			/**zz:[{denfalks:"dak"},{content}]*/
			clickTimeline(topicID) {
				console.log('Vis执行了', topicID)
			},
			/**
			 * 在时间轴上添加标注点
			...
			cycleFun(self) {
				...
				self.timeline.moveTo(self.centerTimeStr, {
					animation: {
						duration: 1000,
